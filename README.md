以下是更新后的 `README.md` 文件，突出 `MouseJigglerPro` 的核心逻辑，重点说明如何防止屏幕熄灭、保持电脑活跃，并对比传统方案阐述其优势。内容涵盖安装、使用方法以及完整文件清单，保持中英文双语支持。

---

```markdown
# MouseJigglerPro / 鼠标抖动器 Pro
...
- **MouseJigglerPro’s Edge / 本方案优势**:  
  1. **Dual-Layer Protection / 双重保护**: Combines software simulation (mouse/keyboard) with Windows API calls for comprehensive activity signaling.  
     - **中文**: 结合软件模拟（鼠标/键盘）和 Windows API 调用，提供全面的活跃信号。  
  2. **Thread Safety / 线程安全**: Uses `threading.Event` and daemon threads to ensure all processes terminate cleanly, avoiding zombie processes common in basic scripts.  
     - **中文**: 使用 `threading.Event` 和守护线程，确保所有进程干净退出，避免简单脚本常见的僵尸进程。  
  3. **User-Friendly / 用户友好**: Modern UI with bilingual support and system tray integration, unlike command-line-only tools.  
     - **中文**: 现代界面支持双语和系统托盘集成，优于仅限命令行的工具。  
  4. **Customizability / 可定制性**: Offers Light and Crazy modes for different use cases, surpassing one-size-fits-all solutions.  
...
1. Clone the repository / 克隆仓库:  
   ```bash
   git clone https://github.com/yourusername/MouseJigglerPro.git
   cd MouseJigglerPro
   ```
...
2. Build the EXE / 打包 EXE:  
   ...
   ```
3. Find the executable in `dist/MouseJigglerPro.exe` / 在 `dist/MouseJigglerPro.exe` 中找到可执行文件。

### How to Use / 操作指南
1. Launch the program / 启动程序: Open `MouseJigglerPro.exe` or run the script / 打开 `MouseJigglerPro.exe` 或运行脚本。  
2. Select language / 选择语言: Choose English or Chinese / 选择英文或中文。  
3. Prevent screensaver / 防止息屏: Click "Light Jiggle" or "Crazy Jiggle" / 点击“轻度抖动”或“疯狂抖动”。  
4. Stop simulation / 停止模拟: Click "Stop" / 点击“停止”。  
5. Exit cleanly / 干净退出: Click "Exit" or close the window (no lingering processes) / 点击“退出”或关闭窗口（无残留进程）。  
6. Status indicator / 状态指示: Green (Light) / Orange (Crazy) / Red (Stopped) / 绿色（轻度）/橙色（疯狂）/红色（停止）。
...
Fork, submit issues, or send pull requests on GitHub! / 欢迎 Fork、在 GitHub 上提交问题或发送 Pull Request！
...
1. **核心逻辑突出**：
   - 详细说明了三层机制（鼠标移动、键盘模拟、Windows API）如何协同防止息屏和保持活跃。
   - 每部分逻辑配有中英文解释，确保清晰易懂。

2. **对比传统方案**：
   - 列出传统方法的局限性（如简单脚本易失效、硬件设备不灵活）。
   - 强调 `MouseJigglerPro` 的双重保护、线程安全、用户友好性和定制性优势。

3. **结构优化**：
   - 将“核心逻辑”作为独立章节置于功能之后、使用之前，吸引技术用户关注。
   - 保持中英文并列，确保全球用户都能理解。
